    ...
    activeConnection(activeBox) {
        ...
        if (activeBox.target.dataset['type'] == 'wifi') {
            ...
            if (!select[0]) {
                ...
                params.push({
                    type: activeBox.target.dataset['type'], activate: activeBox.target.checked, SSID: select[0], password: select[1]
                });
            }
    ...
    getParams(inputs) {
        ...
        if (event.currentTarget.id == 'Submit_network_params0') {
            Object.values(inputs[0]).forEach((param) => {
                ...
                if (param.type == 'checkbox') { select.push(param.checked) }
                else { select.push(param.value) }

            });
            params.push({
                type: select[0], DHCP: select[1], IP: select[2], mask: select[3],
                gateway: select[4], dns1: select[5], dns2: select[6]
            });
            ...
            if (select[7]) {
                params.push({
                    type: select[0], DHCP: select[1], IP: select[2], mask: select[3],
                    gateway: select[4], dns1: select[5], dns2: select[6], SSID: select[7], password: select[8]
                });
            } else {
                return alert('Choose a Wi-Fi network please');
            }
