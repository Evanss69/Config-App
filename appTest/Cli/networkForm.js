import { createElement, createPassword } from './util';
...
class NetworkTableFormUI {
  ...
  createTable(parentEl) {
    this.table = createElement('table', { width: '100%', paddingTop: '20px' }, parentEl);
    let rowIndex = 0;
    this.def.forEach((dataConfigNetwork, r) => {
      ...
      if (r < 2) {
        ...
        this.createRow(rowIndex, connectionType, dataConfigNetwork);
        rowIndex++;
      }
    });
  ...
  createRow(id, connectionType, dataConfigNetwork) {

    const rowEl = createElement('tr', {marginBottom:'20px'}, this.table);
    this.inputs.push([]);
    var content;
    ...
    if (rowEl.children[1].value == 'wifi') {
      ...
      const rowElWifi = createElement('tr', {}, this.table);
    ...
    let underRow = createElement('tr', {}, this.table);
  ...
  createWifiList(parentEl, type, name ,list, inputs, onChange) {
    let inputWifiList = createElement('input', { width: '125px' }, parentEl);
    inputWifiList.name = 'inputWifiList';
    inputWifiList.setAttribute('list', 'wifiList');
    inputWifiList.value=name;
    let wifiList = createElement('datalist', {}, parentEl);
  ...
  createNetworkParamInput(parentEl, type, inputs, id, opts = {}) {
    let el = createElement('input', { width: '125px' }, parentEl);
    inputs.push(el);
  ...
  createDhcpCheckbox(parentEl, type, inputs, id, opts = {}) {
    let dhcpBox = createElement('input', {}, parentEl);
    inputs.push(dhcpBox);
